class GitHubPortfolio {
    ...
    renderRepositories(repos) {
        ...
        if (repoCountEl) {
            repoCountEl.textContent = ownRepos.length;
        }

        // Standard string comparison is faster than creating Date objects for ISO-8601 strings
    ...
    escapeHtml(text) {
        if (!text) return '';
        return text.replace(/[&<>"']/g, m => this.htmlEscapeMap[m]);
    }
